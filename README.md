...
| **HyperGraphTrading (Ours)** | **-5.5%** | **-1.47** | **-9.0%** | **정확 계산 가능** |

**주요 발견**:
- ✅ **리스크 통제 최우수**: MDD -9.0% (모든 베이스라인 대비 우수)
- ⚠️ 수익성 개선 필요: 현재 -5.5% (추가 최적화 진행 중)
- ✅ **추론 속도**: 59,859배 향상 (목표 100배 초과 달성)
- ✅ **승률 계산 로직**: 매수-매도 쌍 기반 정확한 승률 계산 구현 완료 (2025-12-21)

**참고**: 
- 승률 계산 로직이 수정되어 이제 정확한 승률이 계산됩니다 (`src/trading/backtester.py`).
- 실제 백테스팅 실행 시 매수-매도 쌍을 FIFO 방식으로 매칭하여 승률을 계산합니다.
- 거래 비용을 고려한 정확한 승리/패배 판정이 이루어집니다.
- 테스트 검증 완료: 승률 계산 테스트 3개 모두 통과 (100%, 50%, 0% 시나리오)
